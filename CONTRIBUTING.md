...
To get started with the project, run `npm run bootstrap` in the root directory to install the required dependencies for each package as well as the example app
...
To edit the Kotlin files, open `example/android` in Android studio and find the source files at `kustomerreactnative` under `Android`.
